...
function seedIfEmpty() {
  ...
  for (const file of fs.readdirSync(SEED_DIR)) {
    ...
    // Non-destructive upgrade: add any top-level keys the seed has but the
    // existing (possibly older) trip is missing. Never overwrites existing
    // fields, so user edits are preserved.
    try {
      ...
      if (changed) {
        ...
        console.log(`🔧 Upgraded existing trip with new fields: ${file}`);
      }
    } catch (e) {
      console.error(`Could not upgrade ${file}:`, e.message);
    }
  }
}
